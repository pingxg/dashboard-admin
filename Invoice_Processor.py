...
def get_start_and_end_date_from_calendar_week(year, iso_week):
    """
    input: year number, int; iso week number
    output: start date and end date of the week, data type: datetime
    """
    # ISO week date format requires %V instead of %W
    # %V is ISO week number, %W is week of year with Monday as first day
    monday = datetime.datetime.strptime(
        f"{year}-W{iso_week:02d}-1", "%G-W%V-%w"
    ).date()
...
if last_externalID_ns is not None and sok_data is not None:
    ...
    column_mapping = {
        ...
        "kuluttajapakkauskoko": "Consumer pack size",
        "myyntiyksikkö": "Sales Unit",
        "pvm": "Delivery Note Date",
        "aok": "AOK",
        "toimipaikka": "Store",
        "my € sis. alv": "ALV14",
        "my yks": "Quantity",
        "my vol (kg,l)": "My vol (kg,l)",
    }
    df.rename(columns=column_mapping, inplace=True)
    df = df.drop(
        ["Consumer pack size", "My vol (kg,l)", "AOK"], axis=1
    )
    # remove empty rows
    df = df.dropna()
    ...
    df['Delivery Note Date'] = pd.to_datetime(df['Delivery Note Date'], errors="coerce")
    ...
    df["Location (NS)"].loc[
        (df["Store"] == "PRISMA LÄNSIKESKUS TURKU")
        & (df["Delivery Note Date"] >= "2025-03-01")
    ] = "L17 Sushibar Itäharju Prisma Turku"
