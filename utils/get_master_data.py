...
def get_master_data(sheet_name):
    ...
    master_data_df = pd.read_excel(master_data,sheet_name=sheet_name)        # read "SOK" tab
    if sheet_name=="SalesItem":
        master_data_df = master_data_df.dropna(subset=['EAN'])      # delete rows don't have EAN code
        master_data_df['EAN'] = master_data_df['EAN'].astype(int)       # change all the EAN code to integer
