...
location_master_data = (
    LOCATION_MASTER[["Location (NS)", "Internal ID"]].dropna().drop_duplicates()
)
location_master_data_name2id = (
    LOCATION_MASTER[["Ketjuyksikkö (SOK)", "Internal ID"]].dropna().drop_duplicates()
)
product_master_data = PRODUCT_MASTER[
    ["EAN", "Item Category", "Sale Units", "Internal ID PROD", "Display Name/code"]
]
...
def file_uploader(
    label, type="csv", country="FI", sep=";", help=None, allow_multiple=False
):
    df = None
    try:
        data = st.file_uploader(
            f'Upload {label.replace("_"," ")}',
            type=type,
            help=help,
            key=f"{label}_up",
        ...
        if data:
            if type == "csv" or type == "CSV":
                if (
                    label == "month_data"
                    or label == "financial_data"
                    or label == "financial_data_adjustment"
                ):
                    if country == "FI":
    ...
    try:
        if table_name == "sales_data":
            if country == "FI":
                ...
                upload_df["location_internal_id"] = (
                    upload_df["location_internal_name"]
                    .map(
                        dict(
                            zip(
                                location_master_data["Location (NS)"],
                                location_master_data["Internal ID"],
                            )
                        )
                    )
                    .astype(int)
                ...
                upload_df["date"] = pd.to_datetime(
                    upload_df["date_str"], format="%d.%m.%Y"
                ).dt.date
                ...
                current_time = datetime.datetime.now(
                    pytz.timezone(st.secrets["TIMEZONE"])
                )
                upload_df["upload_time"] = current_time
            elif country == "EE":
                upload_df = df.copy().reset_index(drop=True)

                upload_df["unit"] = upload_df["product_name"].map(
                    dict(
                        zip(
                            product_master_data["Display Name/code"],
                            product_master_data["Sale Units"],
                        )
                    )
                )
                upload_df["product_internal_id"] = upload_df["product_name"].map(
                    dict(
                        zip(
                            product_master_data["Display Name/code"],
                            product_master_data["Internal ID PROD"],
                        )
                    )
                )
                upload_df["product_catagory"] = upload_df["product_name"].map(
                    dict(
                        zip(
                            product_master_data["Display Name/code"],
                            product_master_data["Item Category"],
                        )
                    )
                )
                upload_df["location_internal_id"] = upload_df["store_name"].map(
                    dict(
                        zip(
                            location_master_data_name2id["Ketjuyksikkö (SOK)"],
                            location_master_data_name2id["Internal ID"],
                        )
                    )
                )
                ...
                drop_rocca_delivery = upload_df.loc[
                    (upload_df["store_name"] == "PRISMA ROCCA AL MARE")
                    & (upload_df["date"] < pd.to_datetime("2022-09-26").date())
                ].index
                drop_siku_delivery = upload_df.loc[
                    (upload_df["store_name"] == "PRISMA SIKUPILLI")
                    & (upload_df["date"] < pd.to_datetime("2022-12-08").date())
                ].index
                drop_other_delivery = upload_df.loc[
                    (upload_df["store_name"] == "PRISMA TISKRE")
                    | (upload_df["store_name"] == "PRISMA VANALINN")
                    | (upload_df["store_name"] == "PRISMA ROO")
                ].index
                ...
                upload_df["location_internal_id"] = upload_df[
                    "location_internal_id"
                ].astype(int)
                upload_df["product_internal_id"] = upload_df[
                    "product_internal_id"
                ].astype(int)
                upload_df["quantity"] = (
                    upload_df["quantity"]
                ...
                def generate_invoice_text(upload_df):
                    ...
                    for (store_name, year, month), group in grouped:
                        ...
                        total_amount_after_commission = str(
                            group["amount"].sum().round(2)
                        ).replace(".", ",")
                ...
                upload_df = upload_df[
                    ["date", "store_name", "ean", "quantity", "_amount"]
                ]
                upload_df = upload_df[upload_df["ean"].astype(str).str.isdigit()]
                upload_df["ean"] = upload_df["ean"].astype(int)
                upload_df["unit"] = upload_df["ean"].map(
                    dict(
                        zip(
                            product_master_data["EAN"],
                            product_master_data["Sale Units"],
                        )
                    )
                )
                upload_df["product_internal_id"] = upload_df["ean"].map(
                    dict(
                        zip(
                            product_master_data["EAN"],
                            product_master_data["Internal ID PROD"],
                ...
                upload_df["product_catagory"] = upload_df["ean"].map(
                    dict(
                        zip(
                            product_master_data["EAN"],
                            product_master_data["Item Category"],
            ...
            table_name == "financial_data" or table_name == "financial_data_adjustment"
        ):
            if country == "FI" and month is not None and year is not None:
                upload_df = df.copy()
                upload_df.columns = upload_df.columns.str.strip()
                upload_df["Financial Row"] = upload_df["Financial Row"].replace(
                    "  ", np.nan
                )
                upload_df = upload_df.dropna()
                upload_df[["account_id", "account_desc"]] = upload_df[
                    "Financial Row"
                ].str.split(" - ", expand=True)
                upload_df["account_id"] = pd.to_numeric(
                    upload_df["account_id"], errors="coerce"
                )
                upload_df = upload_df.dropna().reset_index()
                upload_df["account_id"] = upload_df["account_id"].astype(int)
                upload_df[upload_df.columns[:-1]] = upload_df[
                    upload_df.columns[:-1]
                ].replace("[\€]", "", regex=True)
                upload_df = upload_df.replace(" ", "", regex=True).replace(
                    ",", ".", regex=True
                )
                ...
                upload_df = pd.melt(
                    upload_df,
                    id_vars=["account_id"],
                    var_name="location_internal_name",
                    value_name="amount",
                )
                if table_name == "financial_data_adjustment":
                ...
                upload_df["account_id"] = (
                    upload_df["country"] + "-" + upload_df["account_id"].astype(str)
                )
                location_master_data_copy = (
                    location_master_data[["Location (NS)", "Internal ID"]]
                    .copy()
                    .dropna()
                )
                ...
                upload_df = upload_df[
                    upload_df["location_internal_name"].str.match(r"^L\d{1,3}\s.+")
                ]

                # Extract the numeric ID and store in 'location_id'
                upload_df["location_id"] = upload_df[
                    "location_internal_name"
                ].str.extract(r"^L(\d{1,3})")
                ...
                upload_df.drop(
                    columns=["location_internal_name", "country"],
                    inplace=True,
                    errors="ignore",
                )
            elif country == "EE":
                ...
                upload_df["account_id"] = (
                    country + "-" + upload_df["Account code"].astype(str)
                )
                upload_df.drop(
                    columns=["Description", "Account name", "Account code"],
                    inplace=True,
                    errors="ignore",
                )
                upload_df = upload_df.melt(
                    id_vars=["location_id", "account_id"],
                    var_name="date_str",
                    value_name="amount",
                )
                upload_df["month"] = pd.to_datetime(
                    upload_df["date_str"], format="%d.%m.%Y"
                ).dt.month
                upload_df["year"] = pd.to_datetime(
                    upload_df["date_str"], format="%d.%m.%Y"
                ).dt.year
                upload_df.drop(columns=["date_str"], inplace=True, errors="ignore")
                ...
                upload_df["amount"] = upload_df["amount"]
                upload_df["account_id"] = (
                    country + "-" + upload_df["account_id"].astype(str)
                )
                upload_df["year"] = year
                upload_df["month"] = month
            ...
            upload_df["job_title"].replace(
                [
                    ...
                    "Courier",
                    "Waiter",
                    "Warehouse assistant and chef",
                ],
                "Chef",
                inplace=True,
            )
            upload_df["hour_type"].replace(
                ["Illness of child"], "Illness", inplace=True
            )
            maraplan_location_master_data = (
                get_master_data("MaraplanData")[
                    ["maraplan_location_name", "location_internal_id"]
                ]
                .dropna()
                .drop_duplicates()
            )
            upload_df["location_internal_id"] = upload_df["location_internal_name"].map(
                dict(
                    zip(
                        maraplan_location_master_data["maraplan_location_name"],
                        maraplan_location_master_data["location_internal_id"],
                    )
                )
            )
            upload_df = upload_df.dropna()
            ...
            upload_df = upload_df.loc[
                upload_df["hour_type"].isin(["Normal", "Training", "Illness"])
            ]
            upload_df["date"] = pd.to_datetime(
                upload_df["date_str"], format="%d.%m.%Y"
            ).dt.date
            ...
            try:
                ...
                upload_df["product_category"] = upload_df[
                    "product_category"
                ].str.lower()
            except:
                pass

            location_bw_name_master_data = (
                LOCATION_MASTER[["bw_approver", "Internal ID"]]
                .dropna()
                .drop_duplicates()
            )

            upload_df["location_internal_id"] = upload_df["bw_approver"].map(
                dict(
                    zip(
                        location_bw_name_master_data["bw_approver"],
                        location_bw_name_master_data["Internal ID"],
                    )
                )
            )
            upload_df = upload_df.dropna()
            ...
            upload_df.loc[
                upload_df["product_category"].str.contains("fish"), "product_category"
            ] = "salmon"
            # Updating date if product_category is "salmon"
            upload_df.loc[
                upload_df["product_category"] == "salmon", "date"
            ...
            if country == "FI":
                ...
                upload_df["amount"] = upload_df["amount"].str.replace(",", ".")
                upload_df["amount"] = upload_df["amount"].str.replace("'", "")
                upload_df["amount"].loc[upload_df["amount"].str.contains("kr")] = (
                    upload_df["amount"]
                    .loc[upload_df["amount"].str.contains("kr")]
                    .str.replace("kr", "")
                    .astype(float)
                    / 10
                )
                ...
                upload_df["location_internal_id"] = (
                    upload_df["Location"].str.extract("(\d+)").astype(int)
                )
                upload_df["account_type"] = upload_df["account_id"].map(
                    dict(
                        zip(
                            account_master_data["account_id"],
                            account_master_data["account_type"].str.capitalize(),
                        )
                    )
                )
                ...
                reverse_vendor = {
                    ...
                    for vendor_type in values
                }

                upload_df = df.copy()
                upload_df = upload_df[
                    ["Vendor", "Doc. No.", "Tr. Date", "Invoice Total ", "Department"]
                ]
                ...
                upload_df.loc[
                    upload_df["vendor"] != "OY Golden Crop AB", "amount"
                ] /= 1.2
                upload_df["sushibar"] = upload_df["sushibar"].fillna(
                    "Sushibar Kristiine"
                )
                upload_df["date"] = pd.to_datetime(upload_df["date"], format="%d.%m.%Y")
                upload_df["location_internal_id"] = (
                    upload_df["sushibar"].map(location).astype(int)
                )
        ...
        if st.secrets["EMAIL_ALERT"]:
            send_email(
                st.secrets["OFFICE_USN"],
                f'Error message when processing file "{filename}" for {table_name.replace("_", " ")}',
                e,
            )
        st.stop()
    return upload_df
...
db_option_up = st.selectbox(
    "Available database table options:", table_options, key="db_option_up"
)
...
if db_option_up != "< Please select one table >":
    ...
    if (
        db_option_up == "month_data"
        or db_option_up == "financial_data"
        or db_option_up == "financial_data_adjustment"
    ):
        country = st.radio(
            "Which country does the data belong to?",
            ("FI", "EE", "NO"),
            horizontal=True,
        )
        if country == "FI":
            ...
            year = col1.number_input(
                "Year", value=datetime.datetime.now().year, step=1, key="year_up"
            )
            month_selected = col2.select_slider(
                "Month", options=month_list, key="month_up"
            )
            month = month_list.index(month_selected) + 1
            type, sep, help, allow_multiple = (
                "csv",
                ",",
                "Download from NetSuite each month.",
                False,
            )
        elif country == "EE":
            location = {
            ...
            ee_store_selected = st.select_slider(
                "Select which store:", options=location.keys(), key="ee_month_up"
            )
            st.session_state["ee_month_up_int_id"] = location[ee_store_selected]
            type, sep, help, allow_multiple = (
                "xlsx",
                None,
                "Download from Merit each month.",
                False,
            )
        elif country == "NO":
            location = {
            ...
            col1, col2 = st.columns([1, 3])
            year = col1.number_input(
                "Year", value=datetime.datetime.now().year, step=1, key="year_up_no"
            )
            month_selected = col2.select_slider(
                "Month", options=month_list, key="month_up_no"
            )
            month = month_list.index(month_selected) + 1
            ...
            type, sep, help, allow_multiple = (
                "xlsx",
                None,
                "Download from Cozone each month.",
                False,
            )
        ...
        if country == "NO":
            type, sep, help, allow_multiple = (
                "xlsx",
                None,
                "File you upload to NetSuite or received raw weekly report.",
                True,
            )
        elif country == "EE":
            type, sep, help, allow_multiple = (
        ...
        if country == "FI":
            type, sep, help, allow_multiple = (
                "csv",
                ";",
                "Download from Netsuite Suite Analytics.",
                False,
            )
        elif country == "EE":
            type, sep, help, allow_multiple = (
    ...
    if df is not None and filename not in st.session_state["upload_history"].keys():
        upload_df = process_data(
            df=df,
            filename=filename,
            table_name=db_option_up,
            country=country,
            month=month,
            year=year,
        )
        ...
        if db_option_up == "financial_data":
                ...
                if filename not in st.session_state["upload_history"].keys():
                    try:
                        ...
                        info_placeholder.error(
                            f"Something is wrong when updating data to the database.",
                            icon="🚨",
                        )
                        st.write(e)
                        if st.secrets["EMAIL_ALERT"]:
                            send_email(
                                st.secrets["OFFICE_USN"],
                                f'Error message when uploading {db_option_up.replace("_", " ")}',
                                e,
                            )
                        st.stop()
                    finally:
                    ...
                    st.error(
                        f"You have already updated this data, please don't upload again!",
                        icon="🚨",
                    )
                    st.stop()
        else:
            upload_btn = st.button(
                "Upload to Database", key=f"{db_option_up}_upload_button"
            )
            ...
            if upload_btn and upload_df is not None:
                if filename not in st.session_state["upload_history"].keys():
                    try:
                        info_placeholder.info(
                            f'Start uploading {upload_df.shape[0]} rows of {db_option_up.replace("_"," ")}...',
                            icon="ℹ️",
                        )
                        ...
                        info_placeholder.error(
                            f"Something is wrong when uploading data to the database.",
                            icon="🚨",
                        )
                        st.write(e)
                        if st.secrets["EMAIL_ALERT"]:
                    ...
                    st.error(
                        f"You have already uploaded this data, please don't upload again!",
                        icon="🚨",
                    )
                    st.stop()
